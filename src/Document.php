	...
	public function createActions($pagename, $pagenode, $actionname = false) {
		...
		for($i = 0; $i < $actionNodes->length; $i++) {
			...
			if($actionNode) {
				...
				if($shortcutNodes && $shortcutNodes->length > 0) {
					$shortcutNode = $shortcutNodes->item(0);

					// Get reffered actionObj
					$refferedpagename = $shortcutNode->getAttribute('page');
					if($refferedpagename && $refferedpagename != '' && $pagename != $refferedpagename) {
						// The shortcut refferes to another page
						if($this->pageExists($refferedpagename)) {
							$refferedpageObj = &$this->sitemap[$refferedpagename];
						} else {
							throw new Exception("Reffered Page ".$refferedpagename." not exists!");
						}
					} else {
						$refferedpageObj = &$this->sitemap[$pagename];
					}
					...
					if($refferedactionname && $refferedactionname != '') {
						...
						if($refferedAction) {
							...
							if($methodNode && $methodNode->length > 0 && $methodNode->item(0)->nodeValue) {
								...
								$method = $refferedAction->getMethod();
								if(!$method) {
									$method = $this->sitemap[$pagename]->getMethod();
								}
							}

							// Wenn der Shortcut einen eigenen class node hat
							$classNode = $xpath->query("class", $actionNode);
							if($classNode && $classNode->length > 0 && $classNode->item(0)->nodeValue) {
								$class = $classNode->item(0)->nodeValue;
							} else {
								$class = $refferedAction->getClass();
								if(!$class) {
									$class = $this->sitemap[$pagename]->getClass();
								}
							}

							// Wenn der shortcut einen eigenen Style hat
							$styleNodes = $xpath->query("style", $actionNode);
							if($styleNodes && $styleNodes->item(0)) {
								if($styleNodes->item(0)->nodeValue != '') {
									$stylesheet = $styleNodes->item(0)->nodeValue;
								}
							} else {
								$stylesheet = $refferedAction->getStylesheet();
							}

							// Das File aus der refferenz muss auch eingebunden werden
							// @todo localFiles erlauben
							if($refferedAction->getFile() && $refferedAction->getFile() != '') {
								$shortcutfile = $refferedAction->getFile();
							}
						}
					}
				}
				...
				$this->sitemap[$pagename]->addAction($actionname, $class, $method, $refferedactionname);
				$refferedactionname = false;
				$shortcutfile = false;
				$class = false;
				$method = false;
				...
				if($shortcutfile) {
					$this->sitemap[$pagename]->addFile($shortcutfile, $actionname);
				}
				...
				if($stylesheet) {
					$this->sitemap[$pagename]->setStylesheet($actionname, $stylesheet);
				}
				...
				// Add Elements from reffered action
				if($refferedAction) {
					$refferedlocals = $refferedAction->getLocals();
					foreach($refferedlocals as $arrname => $arr) {
						$this->sitemap[$pagename]->setLocal($actionname, $index, $arr);
					}
				}
