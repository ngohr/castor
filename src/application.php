...
class Application {
	...
	public function getPage() {
		return $this->sitemap[$this->pagename];
	}

	public function setStylesheet($file) {
		$pageObj = $this->getPage();
		$pageObj->setStylesheet($this->actionname, $file);
			...
			foreach($this->sitemap as $name => $obj) {
				...
				if($computeActions) {
					...
					foreach($arrActions as $action => $name) {
						...
						if($name == $this->getActionname())
							$elementAction->setAttribute('active', 'true');
	...
	public function getSitemapArray($computeActions = true) {
		...
		foreach($this->sitemap as $pagename => $obj) {
			if($pagename == $this->getPagename())
				...
				$active = false;
			$arr = array('name' => $pagename, 'title' => $obj->getTitle(), 'index' => $obj->getIndex(), 'active' => $active);
			if($constants = $obj->getConstants()) {
				foreach($constants as $constantName => $constantValue) {
	...
	// Construct sitemap/page elements as model, returns Array
	public function getSitemapAssoc($computeActions = true) {
		if(!$this->sitemap)
			return false;
		...
		foreach($this->sitemap as $pagename => $obj) {
			...
			$arrSitemap['page'][$pagename] = $arr;

			if($computeActions) {
				$arrSitemap['page'][$pagename]['action'] = array();
				$arrActions = $obj->getActions();
				foreach($arrActions as $idx => $name) {
					$objAction = $obj->getObjAction($name);
					if($name == $this->getActionname())
						...
						$active = false;
					$arrSitemap['page'][$pagename]['action'][$name] = array('name' => $name, 'title' => $objAction->getTitle(), 'active' => $active);
					if($constants = $objAction->getConstants()) {
						foreach($constants as $constantName => $constantValue) {
							$arrSitemap['page'][$pagename]['action'][$name][$constantName] = $constantValue;
						}
					}
				}
			}
